...
mermaid渲染问题：在渲染md文件中的mermaid图表时遇到了一些问题。
  ...
  5. SVG闪烁：由于SVG也有一个初始化过程，导致图表的渲染有一个从初始SVG到最终SVG的过程，**解决方案**是SVG仅能使用固定大小，所以设置`height:50vh`，目前开发环境感受是加载mermaid图表时会有一点点卡顿的感觉，使用animation关键帧动画尽量优化了一下

关于博客按需加载问题：如果使用数据库存储博客数据的话确实可以做到大部分情况下性能相比于文件系统全量加载更优，但是会导致系统复杂度上升，如果后续博客数量太多可以考虑改造成数据库

博客修改功能设计：
 - 需要设计API来GET与PUTblog，通过请求修改文档内容
 - 在线编辑md文件使用插件实现，目前看该插件不支持显示mermaid图表，并且部分语法格式不对，后续尝试自行支持
 - 新增博客功能实现逻辑与修改类似，区别在于需要新建文件

修改后点击博客进入文章无法滚动问题，目前看修改文件后详情能够正常更新，但是不显示滚动条
 - 尝试添加min-h-screen解决，未解决
